...
class KewirausahaanController extends Controller
{
    ...
    function store(UsahaRequest $req)
    {
        ...
        if ($req->hasFile('legalitas_usaha')) {
            ...
            $data['legalitas_usaha'] = $legalitasUsaha->store('assets/legalitas_usaha', ['disk' => 'public_upload']);
        } else {
        ...
        if ($req->foto_produk) {
            ...
            foreach ($req->file('foto_produk') as $fotoProduk) {
                $data['foto_produk'][] = $fotoProduk->store('assets/foto_produk', ['disk' => 'public_upload']);
            }
        } else {
            unset($data['foto_produk']);
        }
